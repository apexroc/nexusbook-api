...
1. **GitHub Pages 构建**:
   - 使用 `BASE_PATH=/nexusbook-api` 构建文档
   - 推送到 `gh-pages` 分支
   - 自动部署到 GitHub Pages

2. **K3s 构建**:
   - 使用根路径（无 BASE_PATH）构建文档
   - 推送到 `openapi` 分支
   - Docker 镜像从 `openapi` 分支拉取预构建文档
   - 构建轻量级 Nginx 镜像
   - 推送到 GitHub Container Registry

### 优势

- ✅ **快速构建**: Docker 不需要安装 Node.js 和构建工具
- ✅ **小镜像**: 只包含 Nginx 和静态文件
- ✅ **单一 Action**: 一个 workflow 管理所有部署
- ✅ **灵活部署**: 支持选择性部署到不同环境
...
### 方式二: 手动部署 (GitHub Actions)

在 GitHub Actions 页面，点击 "Run workflow"，可以选择:

1. **github-pages**: 仅构建并部署到 GitHub Pages
2. **k3s**: 仅构建文档、镜像并部署到 K3s 集群
3. **both**: 同时部署到两个环境
