...
- 带排序与过滤
  - curl 'https://open.nexusbook.com/api/v1/doc/product/123/data?sort=name:asc&filter=status:eq:active'
- 深分页（基于游标）
  - 首次请求：curl 'https://open.nexusbook.com/api/v1/doc/product/123/data?pageSize=100'
  - 下一页：curl 'https://open.nexusbook.com/api/v1/doc/product/123/data?pageSize=100&cursor=...'
...
cURL 示例
- 基础结构化查询
  - curl -X POST 'https://open.nexusbook.com/api/v1/doc/product/123/data/query' -H 'Content-Type: application/json' -d '{
...
- 嵌套过滤
  - curl -X POST 'https://open.nexusbook.com/api/v1/doc/product/123/data/query' -d '{
    "filters": {
      "logic": "or",
      "conditions": [
...
- 列表查询（简单）
  - curl 'https://open.nexusbook.com/api/v1/doc/product/123/data?page=1&pageSize=20'
- 列表查询（深分页）
  - 首次：curl 'https://open.nexusbook.com/api/v1/doc/product/123/data?pageSize=100'
  - 下一页：curl 'https://open.nexusbook.com/api/v1/doc/product/123/data?pageSize=100&cursor=...'
- 结构化查询（基础）
  - curl -X POST 'https://open.nexusbook.com/api/v1/doc/product/123/data/query' -H 'Content-Type: application/json' -d '{
    "filters": {"logic": "and","conditions": [{"field": "status","operator": "eq","value": "active"}]},
