...
# 获取订单的完整信息包
curl -H 'Authorization: Bearer TOKEN' \
  'https://open.nexusbook.app/api/v1/doc/purchaseOrder/123?include=metadata,views,data,comments,revisions'
```
...
# 查询订单的所有产品
curl 'https://open.nexusbook.app/api/v1/doc/order/123/relations?targetDocType=product'
```
...
```bash
# 批量更新多个产品的价格
curl -X POST 'https://open.nexusbook.app/api/v1/doc/product/123/data/bulk' \
  -H 'Authorization: Bearer TOKEN' \
  -H 'Content-Type: application/json' \
  -d '[
    {
...
```bash
# 批量创建多个关联
curl -X POST 'https://open.nexusbook.app/api/v1/doc/order/123/relations/batch' \
  -H 'Authorization: Bearer TOKEN' \
  -H 'Content-Type: application/json' \
  -d '[
    {
